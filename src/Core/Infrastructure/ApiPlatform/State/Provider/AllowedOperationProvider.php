...
class AllowedOperationProvider implements ProviderInterface
{
    /**
    ...
    {
        if (key_exists('id', $uriVariables) && $uriVariables['id'] != '$') {
            /**
        ...
        foreach ($reflection->getAttributes(ApiResource::class) as $attribute) {
            ...
            foreach ($apiResource->getOperations() as $op) {
                /**
                 * @var \ApiPlatform\Metadata\HttpOperation
                 */
                $operation = $op;
                ...
                if ($processor) {
                    ...
                    if (in_array(needle: CommandProcessor::class, haystack: class_parents(object_or_class: $processor))) {
                        ...
                        foreach (call_user_func(callback: "$processor::usedCommandRequests") as $commandRequest) {
                            //default voter result when no voter exists
                            //reject when operation need id but don't have it
                            $voted = (str_contains($operation->getUriTemplate(), '{id}') && !is_null($id)) ||
                                (!str_contains($operation->getUriTemplate(), '{id}') && is_null($id));
        ...
        $allowed->commands = array_values(array_unique($allowed->commands));
        return $allowed;
    }
}